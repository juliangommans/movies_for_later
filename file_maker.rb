...
class BBMbuilder
  ...
  def build_setup_routes
    ...
    @components = "#{@base_route}/components"
    ...
    routes = [@base_route, @config, @marionette, @backbone, @components, @entities, @entities_base, views, @views_base, @controllers]
  ...
  def app_components
    ["controller", "view"].each do |item|
      File.open("#{@components}/loading_#{item}.js.coffee", "w+") { |f| f.write(self.send("loading_#{item}")) } unless File.file?("#{@components}/loading_#{item}")
    end
  end
...
<% end %>"
  ...
  def loading_controller
    "@#{@title}.module 'Components.Loading', (Loading, App, Backbone, Marionette, $, _) ->
